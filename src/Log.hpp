...
*/
  void writeRequest(size_t id, std::string request, std::string ipAddress) {
    std::string requestReceived =
        std::to_string(id) + ": " + request + " from " + ipAddress + "@" + getGMtime(time(0));
  ...
  void writeinCacheExpired(size_t id,time_t inputTime){
     std::string toWrite = std::to_string(id) + ": in cache, but expired at " +getGMtime(inputTime);
     writeToFile(toWrite);
  }
  ...
  void writeCached(size_t id,time_t inputTime){
    std::string toWrite = std::to_string(id) + ": cached, expires at "+ getGMtime(inputTime);
    writeToFile(toWrite);
  }
};